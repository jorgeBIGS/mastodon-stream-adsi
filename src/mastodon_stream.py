...
import os
from mastodon_credenciales import *
...
quiet = False
watchdog = False
topic_name_batch, producer_batch = '' , ''
topic_name_speed, producer_speed = '' , ''
...
class Listener(mastodon.StreamListener):
    ...
    def on_update(self, status):
        ...
        if enable_kafka:
            producer_speed.produce(topic = topic_name_speed, value = value_dict)
            producer_batch.produce(topic = topic_name_batch, value = value_dict)
            
            producer_speed.flush()
...
def main():
    ...
    global quiet
    global watchdog
    global topic_name_batch, producer_batch
    global topic_name_speed, producer_speed
    ...
    if enable_kafka:
        topic_name_batch, producer_batch = kafka_producer('mastodon-topic-batch')
        topic_name_speed, producer_speed = kafka_producer('mastodon-topic-speed')

        

    mastodon = Mastodon(access_token = secret_token, api_base_url = base_url)
